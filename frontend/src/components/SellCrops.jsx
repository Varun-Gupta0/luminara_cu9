...
/*
  Sell Entitys module:
  - List Entitys/produce by district
  - Currency: Rs
...
export default function SellEntitys() {
  const ref = useRef(null);
  useEffect(() => { if (ref.current) ref.current.classList.add("visible"); }, []);
  const [listings, setListings] = useState([]);
  const [selectedLocation, setSelectedLocation] = useState("");
  const [form, setForm] = useState({ item: "", quantity: "", price: "", district: "" });

  useEffect(() => {
    if (selectedLocation) {
      const raw = localStorage.getItem(`farmigo_sell_listings_${selectedLocation}`);
  ...
  function submit(e) {
    e.preventDefault();
    if (!form.item || !form.quantity || !form.price || !form.district) {
    ...
    setListings(updated);
    localStorage.setItem(`farmigo_sell_listings_${form.district}`, JSON.stringify(updated));
    setForm({ item: "", quantity: "", price: "", district: "" });
  }

  return (
    <section className="card" ref={ref}>
      <h2 style={{marginTop:0}}>Sell Entitys</h2>
      <p style={{color:"var(--muted)"}}>List your crops/produce for sale in your district.</p>

      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:12}}>
        {/* Listing Section */}
        <div>
          <h3 style={{marginBottom:8}}>Create Listing</h3>
        ...
        {/* Viewing Section */}
        <div>
          <h3 style={{marginBottom:8}}>Listings in {selectedLocation || "Select a district"}</h3>
            ...
            <div key={l.id} style={{padding:10,background:"rgba(255,255,255,0.02)",borderRadius:8,marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  ...
                  <div style={{color:"var(--muted)",fontSize:13}}>{l.quantity} • Rs {l.price}</div>
                </div>
