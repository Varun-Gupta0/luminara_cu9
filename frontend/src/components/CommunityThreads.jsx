...
/*
  ...
  - Location filtering based on selected Indian district.
...
export default function CommunityThreads() {
  ...
  useEffect(() => {
    if (selectedLocation) {
      const raw = localStorage.getItem(`farmigo_community_threads_${selectedLocation}`);
  ...
  function submit(e) {
    ...
    if (!form.title || !form.description || !form.location) {
      alert("Fill title, description, and select a location");
      return;
    }
    const thread = {
    ...
    setThreads(updated);
    localStorage.setItem(`farmigo_community_threads_${form.location}`, JSON.stringify(updated));
    setForm({ title: "", description: "", location: "" });
  }

  // Filter threads to show only from selected location
  const nearbyThreads = threads;
  ...
  return (
    <section className="card" ref={ref}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:12}}>
        {/* Posting Section */}
        <div>
          <h3 style={{marginBottom:8}}>Post a Thread</h3>
          <form onSubmit={submit} style={{display:"grid",gap:8}}>
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <button className="btn btn-ghost" type="button" onClick={()=>setForm({title:"",description:"",location:selectedLocation})}>Reset</button>
            </div>
          </form>
        </div>
        ...
        {/* Viewing Section */}
        <div>
          <h3 style={{marginBottom:8}}>Threads in {selectedLocation || "Select a district"}</h3>
