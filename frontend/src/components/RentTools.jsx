...
/*
  Rent Tools module:
  - List machinery/equipment by district
  - Request to rent with hours and date
  - Saved to localStorage for demo, keyed by district
  - Currency: Rs
*/

// Comprehensive list of Indian districts grouped by state
const INDIAN_DISTRICTS = {
  ...
  "Nagaland": ["Chümoukedima", "Dimapur", "Kiphire", "Kohima", "Longleng", "Mokokchung", "Mon", "Niuland", "Noklak", "Peren", "Phek", "Shamator", "Tuensang", "Wokha", "Zunheboto"],
...
export default function RentTools() {
  ...
  const [form, setForm] = useState({ name: "", pricePerhour: "", district: "" });
  ...
  useEffect(() => {
    if (selectedLocation) {
      const raw = localStorage.getItem(`farmigo_rent_listings_${selectedLocation}`);
  ...
  function submitListing(e) {
    e.preventDefault();
    if (!form.name || !form.pricePerhour || !form.district) {
    ...
    setListings(updated);
    localStorage.setItem(`farmigo_rent_listings_${form.district}`, JSON.stringify(updated));
    setForm({ name: "", pricePerhour: "", district: "" });
  }
  ...
  return (
    <section className="card" ref={ref}>
      ...
      <p style={{color:"var(--muted)"}}>List your machinery/equipment for rent or apply to rent from others in your district.</p>

      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:12}}>
        {/* Listing Section */}
        <div>
          <h3 style={{marginBottom:8}}>List Equipment</h3>
          <form onSubmit={submitListing} style={{display:"grid",gap:8}}>
            <select value={form.district} onChange={(e)=>setForm({...form,district:e.target.value})}
              style={{padding:10,borderRadius:8}}>
              <option value="">Select District</option>
              {Object.entries(INDIAN_DISTRICTS).map(([state, districts]) => (
            ...
            <input placeholder="Equipment Name (e.g., Tractor)"
              value={form.name} onChange={(e)=>setForm({...form,name:e.target.value})}
              style={{padding:10,borderRadius:8}} />
            <input placeholder="Price per hour (Rs)"
              type="number" value={form.pricePerhour} onChange={(e)=>setForm({...form,pricePerhour:e.target.value})}
              style={{padding:10,borderRadius:8}} />
            <div style={{display:"flex",gap:8}}>
              <button className="btn btn-primary" type="submit">List Equipment</button>
              <button className="btn btn-ghost" type="button" onClick={()=>setForm({name:"",pricePerhour:"",district:""})}>Reset</button>
            </div>
          </form>
        </div>
        ...
        {/* Renting Section */}
        <div>
          <h3 style={{marginBottom:8}}>Rent Equipment in {selectedLocation || "Select a district"}</h3>
          <select value={selectedLocation} onChange={(e)=>setSelectedLocation(e.target.value)}
            style={{padding:10,borderRadius:8,marginBottom:12,width:"100%"}}>
            <option value="">Select District</option>
            {Object.entries(INDIAN_DISTRICTS).map(([state, districts]) => (
              <optgroup key={state} label={state}>
                {districts.map(district => (
          ...
          {listings.length === 0 && <div style={{color:"var(--muted)"}}>No equipment listed yet</div>}
          {listings.map((t) => (
            <div key={t.id} style={{padding:12,background:"rgba(255,255,255,0.02)",borderRadius:8,marginBottom:8}}>
              <div style={{fontWeight:700}}>{t.name}</div>
              <div style={{color:"var(--muted)",fontSize:13}}>Price: Rs {t.pricePerhour}/hour</div>
      ...
      {selected && (
        <div style={{marginTop:12,padding:12,background:"linear-gradient(180deg, rgba(255,255,255,0.01), transparent)",borderRadius:8}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center",marginTop:8,flexWrap:"wrap"}}>
            ...
            <input type="date" value={date} onChange={(e)=>setDate(e.target.value)} style={{padding:8,borderRadius:8}} />
            <div style={{marginLeft:"auto",fontWeight:700}}>Total: Rs {(selected.pricePerhour * (Number(hours)||1)).toFixed(2)}</div>
